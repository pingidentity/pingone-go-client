	...
	if s := cfg.Service; s != nil {
		if p := s.PingOne; p != nil {
			...
			if v := p.Endpoint.TopLevelDomain; v != nil && *v != "" {
				...
				cfg.SetAuthServerVariableDefaultValue(0, "tld", *v)
				cfg.SetDefaultServerIndex(0)
				cfg.SetDefaultAuthServerIndex(0)
			}

			if v := p.Endpoint.RootDomain; v != nil && *v != "" {
				cfg.SetServerVariableDefaultValue(1, "domain", fmt.Sprintf("api.%s", *v))
				cfg.SetAuthServerVariableDefaultValue(1, "domain", fmt.Sprintf("auth.%s", *v))
				cfg.SetDefaultServerIndex(1)
				cfg.SetDefaultAuthServerIndex(1)
			}
			...
			if v := p.Endpoint.AuthDomain; v != nil && *v != "" {
				cfg.SetAuthServerVariableDefaultValue(1, "domain", *v)
				cfg.SetDefaultAuthServerIndex(1)
			}

			if v := p.Endpoint.CustomDomain; v != nil && *v != "" {
				// Only auth supports custom domains
				cfg.SetAuthServerVariableDefaultValue(2, "domain", *v)
				cfg.SetDefaultAuthServerIndex(2)
			}

			if v := p.Endpoint.AuthEnvironmentID; v != nil && *v != "" {
				cfg.SetAuthServerVariableDefaultValue(0, "environmentId", *v)
				cfg.SetAuthServerVariableDefaultValue(1, "environmentId", *v)
			}
			...
			if p.Auth.AccessToken == nil || *p.Auth.AccessToken == "" {
				...
				if p.Auth.ClientID != nil && *p.Auth.ClientID != "" && p.Auth.ClientSecret != nil && *p.Auth.ClientSecret != "" {
					authEndpoint, err := cfg.AuthServerURLWithContext(ctx, "pingone")
					if err != nil {
						return nil, err
					}
					...
					config := clientcredentials.Config{
						...
						TokenURL:     fmt.Sprintf("%s/as/token", authEndpoint),
